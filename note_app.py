# Note App
# Create and store data for later purpose.
# Features:
# 1. User can create a note.
# 2. User can edit a note.
# 3. User can delete a note.
# 4. When the user returns, the data will be retrieved.

# making app by using gui(tkinter) and database in python.
...
def onselect(evt):
    ...
    w = evt.widget  # here Tkinter passes an event object to onselect()
...
Label(top_frame, text= "Notes Folder", font=("arial", 20, "bold","underline")).pack()
list_notes = Listbox(top_frame, height=8, width=70)
list_notes.bind('<<ListboxSelect>>', onselect)
list_notes.pack(pady=(10, 10))

scroll_list.config(command=list_notes.yview)
list_notes.config(yscrollcommand=scroll_list.set, cursor="hand2", background="#4E5180", fg="white", highlightbackground="black", bd=0, selectbackground="black")
top_frame.pack(side=tk.TOP, padx=(0, 5))

text_frame = tk.Frame(root)
note_title = tk.Entry(text_frame, width=20, font=("arial", 16, "bold"))
note_title.insert(tk.END, "Title")
note_title.config(background="#98AFC7", highlightbackground="lavender")
...
note_text = tk.Text(text_frame, height=7, width=40, font=("arial", 16, "bold"))
note_text.pack(side=tk.TOP, fill=tk.Y, padx=(5, 0), pady=(0, 5))
note_text.tag_config("tag_your_message", foreground="purple")
note_text.insert(tk.END, "Notes")
scroll_text.config(command=note_text.yview)
note_text.config(yscrollcommand=scroll_text.set, background="#565051",fg="#ffffff", highlightbackground="lavender")
...
photo_delete = PhotoImage(file="del.png")
